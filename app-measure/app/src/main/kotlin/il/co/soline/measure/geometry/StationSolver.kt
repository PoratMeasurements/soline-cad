...
/**
 * StationSolver — הליבה-המתמטית של **מדידת-P2P האוטומטית של Leica X6** (CVSM §6.1/§6.4/§7.6).
 *
 ...
 * בשיטת "משולש-הזהב" (law of cosines על אלכסון-הפינה) — בלי זווית-אופקית מהמכשיר.
 *
 ...
 * כיוון-הקיר ממצפן. במקום זה בונים אותו גאומטרית, בדיוק כפי ש-CVSM עושה
 * (`goldenTriangleA/B/C`, `goldenTriangleAngle`, `WallCornerAngles`):
 *
 ...
 *
 * ── כיול-עמדה (station calibration, §7.6) ─────────────────────────────────────
 ...
 * (מחוץ-לתחום גל-זה): רילוקציה מרובת-עוגנים — 4 עוגני-ייחוס והעברת-הלייזר (CVSM §7.6,
 * `anchorCalibrationPhase`) לפתרון עמדה-חדשה בטרילטרציה. הבסיס לכך כבר קיים ב-
...
object StationSolver {
    ...
    /**
     * **כייל עמדה** — קובע את מיקום-המכשיר כראשית-החדר. גרסת עמדה-בודדת (§7.6):
    ...
    /** אוריינטציית ירית-P2P — קובעת מה המרחק מייצג (CVSM `isVerticalTarget`/`isNearHorizontal`). */
     ...
     *
     * אם זמינה זווית-אנכית ([vAngleDeg]) מהלייקה, מיושם היטל-אמיתי (CVSM §6.12 "היטל
    ...
    /** היטל-אופקי אמיתי של ירייה (CVSM §6.12) — אורך-קיר בתוכנית מתוך מרחק+זווית-אנכית. */
    ...
    /**
     * **P2P אמיתי** — ממיר ירייה-כדורית לנקודת-תוכנית 2D יחסית-לתחנה (CVSM §6.1;
     * HANDOFF "DST 360-X → XYZ מלא"). כל פינה שנורית מהתחנה נופלת ישירות למקומה
     * בתוכנית — בלי משולש-הזהב ובלי מדידות-אלכסון נוספות:
     *
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // משולש-הזהב — פתרון-זווית-פינה גאומטרי
    // ──────────────────────────────────────────────────────────────────────────
    ...
    /**
     * **משולש-הזהב** — הזווית-הפנימית בפינה B משלושת-אורכי-המשולש (CVSM §6.4,
     * `goldenTriangleA/B/C` → `goldenTriangleAngle`). זו הליבה של "בניית-הזוויות":
     *
     ...
     * ("קלט זווית ידני", CVSM §6.10) או כל זווית שהוזנה ידנית.
     *
     ...
     * (משולש-הזהב), או נלקחת מ-[explicitInteriorDeg] אם ניתנה (למשל "זווית-ישרה 90°").
     */
     ...
     * **על הקיר הקודם** נקבעת מזווית-הפינה שלפני הקיר-הנוכחי (משולש-הזהב/זווית-מפורשת).
     *
        ...
        for (i in 1 until inputs.size) {
            ...
            val turn = when {
                ...
                inp.diagonalToPrev != null ->
                    goldenTriangleAngle(inputs[i - 1].lengthMm, inp.lengthMm, inp.diagonalToPrev, ccw).turnDeg
    ...
    fun solveCornerOnto(
        ...
        val solve = goldenTriangleAngle(prevWall.length, thisLen, diagonal, ccw)
    ...
    /** **חיבור-T** בין קירות סמוכים — מצמיד קצה-קיר אל השכן ("הראשון לא זז"). §5.5. */
    fun tJoin(walls: List<WallEntity>, tolMm: Double = 50.0): List<WallEntity> =
        WallCloseTools.attachTJoin(walls, tolMm)

    /** **סגירת-היקף אוטומטית** — מאריך/מסובב את הקיר-האחרון עד לראשית. §5.7. */
